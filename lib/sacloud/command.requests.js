				...
				switch (resource) {
					...
					case 'sshkey':
						...
						case 'gslb-ping':
 						  ...
 						  var protocol = resource.replace("gslb-" , "")

						  var hc = {};
							if(protocol == "http" || protocol == "https"){
								if (args.shift() === 'path')   hc.Path = args.shift();
								if (args.shift() === 'status') hc.Status = args.shift();
						  }
							if(protocol == "tcp"){
								if (args.shift() === 'port')   hc.Port = args.shift();
							}
							hc.Protocol = protocol;
							
							var gslb = {HealthCheck : hc};
							gslb.DelayLoop = 10;//default:10s
							
							var res = {Settings:{GSLB:gslb}};
							res.Provider = {Class: 'gslb'};
						...
						case 'simplemon-http':
						case 'simplemon-https':
						case 'simplemon-tcp':
						case 'simplemon-ping':

 						  var protocol = resource.replace("simplemon-" , "");
						  ...
						  var hc = {};
							...
							var simplemon = {HealthCheck : hc};
							simplemon.DelayLoop = 60;//default:60s
							simplemon.NotifyEmail = { Enabled : "True"}; // default: email only.
							simplemon.NotifySlack = { Enabled : "False"};
							
							simplemon.Enabled = "True";
							
							var res = {Settings:{SimpleMonitor:simplemon}};
							res.Provider = {Class: 'simplemon'};
								
							if (args.shift() === 'target') res.Status = {Target : args.shift()} ;
							res.Name = res.Status.Target; //default : target ip or hostname.
